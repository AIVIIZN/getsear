...
describe('CRM-V1.2 guest API contract', () => {
  ...
  it('ships POS guest lookup and order attachment contracts', () => {
    ...
    expect(guestCard).toContain('/api/crm/loyalty/checkout')
  ...
  it('wires CRM loyalty into POS checkout enrollment, reward redemption, and receipts', () => {
    const checkoutRoute = read('src/app/api/crm/loyalty/checkout/route.ts')
    const guestCard = read('src/components/pos/GuestAttachmentCard.tsx')
    const orderPanel = read('src/components/pos/OrderPanel.tsx')
    const receiptRoute = read('src/app/api/integrations/email/receipt/route.ts')
    const receiptTemplate = read('src/lib/integrations/email-templates.ts')

    expect(checkoutRoute).toContain('crmCheckoutLoyaltyActionSchema')
    expect(checkoutRoute).toContain("action === 'enroll'")
    expect(checkoutRoute).toContain('redeemReward')
    expect(checkoutRoute).toContain('validateManagerPinForAction')
    expect(checkoutRoute).toContain("from('order_discounts')")
    expect(checkoutRoute).toContain('recalculateOrderTotals')
    expect(checkoutRoute).toContain('crm_loyalty_receipt')
    expect(guestCard).toContain('/api/crm/loyalty/checkout')
    expect(guestCard).toContain('Receipt CTA')
    expect(guestCard).toContain('Manager PIN')
