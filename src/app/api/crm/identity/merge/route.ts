...
export async function POST(request: NextRequest) {
  ...
  if (!candidate) {
    return NextResponse.json({ error: 'Merge candidate evidence is required' }, { status: 404 })
  }
  if (Number(candidate.confidence) < 75) {
    return NextResponse.json({ error: 'Weak matches are suggestion-only and cannot be merged' }, { status: 409 })
  }
