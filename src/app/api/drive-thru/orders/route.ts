...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.json({ error: 'Failed to fetch drive-thru orders' }, { status: 500 })
  }
...
export async function POST(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.json({ error: 'Failed to create drive-thru order' }, { status: 500 })
  }
