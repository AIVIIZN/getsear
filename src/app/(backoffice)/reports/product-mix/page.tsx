...
import { getMockPMIX, type PMIXItem } from '@/lib/reports/mock-data'
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Product Mix (PMIX)</h1>
          {isMock && (
      ...
      <div className="grid grid-cols-4 gap-4">
        {(['Star', 'Plowhorse', 'Puzzle', 'Dog'] as const).map((cls) => (
          <Card key={cls} className="shadow-warm-sm">
            <CardContent className="p-4 flex items-center gap-3">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: CLASSIFICATION_COLORS[cls] }} />
              <div>
                <p className="text-xs text-[var(--muted-foreground)]">{cls}s</p>
                <p className="text-lg font-bold tabular-nums">{classificationCounts[cls] ?? 0}</p>
              </div>
            </CardContent>
          </Card>
        ))}
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {([
                    ...
                    { field: 'quantity_sold' as SortField, label: 'Qty Sold' },
                    { field: 'revenue' as SortField, label: 'Revenue' },
                    { field: 'food_cost_pct' as SortField, label: 'Food Cost %' },
                    { field: 'margin_pct' as SortField, label: 'Margin %' },
                    { field: 'classification' as SortField, label: 'Classification' },
                  ]).map((col) => (
                    <th
                      key={col.field}
                      className={`py-2 px-3 font-medium text-[var(--muted-foreground)] cursor-pointer hover:text-[var(--foreground)] ${col.field === 'name' ? 'text-left' : 'text-right'}`}
              ...
              <tbody>
                {sorted.map((item) => (
                  <tr key={item.name} className="border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--secondary)]">
                    <td className="py-2 px-3">
                      <div className="font-medium">{item.name}</div>
                      <div className="text-xs text-[var(--muted-foreground)]">{item.category}</div>
                    </td>
                    ...
                    <td className="py-2 px-3 text-right">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${CLASSIFICATION_BG[item.classification]}`}>
                        {item.classification}
                      </span>
                    </td>
                  </tr>
