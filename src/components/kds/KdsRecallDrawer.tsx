...
export function KdsRecallDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end">
      ...
      {/* Drawer */}
      <div className="relative z-10 flex h-full w-full max-w-md flex-col bg-[var(--color-kds-surface)] shadow-2xl">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className="mb-3 h-12 w-12 text-[var(--color-kds-text-disabled)]">
                <path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" />
                <path d="M3 3v5h5" />
              </svg>
              <p className="text-sm text-[var(--color-kds-text-muted)]">
                No recently bumped tickets to recall.
              </p>
              <p className="mt-1 text-xs text-[var(--color-kds-text-subtle)]">
            ...
            <div className="space-y-2">
              {recentBumps.map((bump) => (
                <button
                  ...
                  className="btn-press flex w-full items-center justify-between rounded-lg border border-[var(--color-kds-border-strong)] bg-[var(--color-kds-surface-active)] p-3 text-left transition-colors hover:bg-[var(--color-kds-surface-hover)]"
                  style={{ minHeight: 48 }}
                >
                  <div>
                    ...
                    {bump.event_type === 'refire' && bump.reason && (
                      <p className="mt-0.5 text-caption-1 font-semibold text-[var(--color-kds-priority-refire)]">
                        Re-fired: {bump.reason.replace(/_/g, ' ')}
                      </p>
                    )}
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-[var(--color-kds-text-subtle)]">
                      Bumped {formatTimeAgo(bump.bumped_at)}
                    </p>
                    <span className="text-sm font-semibold text-[var(--color-primary)]">Recall</span>
                  </div>
                </button>
              ))}
            </div>
